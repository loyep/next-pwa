...
const baseActiveBoxClassName =
  "items-center justify-between gap-2 text-left w-full flex rounded text-sm transition-colors [word-break:break-word]";
...
const activeActiveBoxClassName =
...
export const ActiveBox = ({ href, className, ...rest }: ActiveBoxProps) => {
  ...
  return (
    <div
      className={clsx(
        baseActiveBoxClassName,
        isActive ? activeActiveBoxClassName : hoverActiveBoxClassName,
        className
      )}
      {...rest}
    />
