...
`next-pwa` has built-in offline support for both Pages Router and App Router. By default, it will cache HTML requests, RSC requests, and RSC precaches
separately. This behavior can be overriden with `workboxOptions.runtimeCaching`, however.
...
If you are seeing the behavior where your app is reloading infinitely, it might be because you have disabled `next-pwa` from building a service worker in
development mode; in which case, try unregistering the service worker if it exists. It might have been installed by a production build of your app before.
...
To enable this behavior, simply add a `/_offline.tsx` file to `pages/` or a `/~offline/page.tsx` file to `app/`. You are all set!
